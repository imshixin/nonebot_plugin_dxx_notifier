...
LastEditors"imsixn
LastEditTime"2022-04-05 23:20:17
...
dxx_priv_notify = on_command('pnotice',aliases={'个人通知',},permission=SUPERUSER)
...
async def init_member(bot: Bot):
    ...
    if not path.exists():
        await tools.write_members_info(path,plugin_config.dxx_notifier_group_id,bot)
        await dxx_init.send(
            r'''第一次初始化，请补充目录下的member_list.json中QQ号对应的人员姓名并填在‘name’,然后再发送一次 /初始化
                 例如
                {
                    "name": "张三",
                    "card_name": "群名片",
        ...
        return
    global member_list
    member_list = await tools.read_members_info(path)
    ...
    await dxx_init.send("-----初始化完成,当前读取的人员有：\n\n姓名 | 网名 | QQ号\n" + message1+"\n-----忽略的QQ有\n\n"+message2)
...
async def check_study_state(bot:Bot):
    ...
    if len(member_list['filterd']) ==0:
        ...
        return
    notice_list = await tools.get_notice_member_list(plugin_config.dxx_username,plugin_config.dxx_password,member_list['filterd'])
...
async def send_group_notify():
    await schedule.week0_12_sender('',plugin_config.dxx_notifier_group_id)
...
async def view_studyed_list(bot:Bot):
    """ 看看那些人做了 """
    mem_list = await tools.get_dxx_studyed_member(plugin_config.dxx_username,plugin_config.dxx_password)
