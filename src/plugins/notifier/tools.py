...
LastEditors"imsixn
LastEditTime"2022-04-07 15:33:32
...
async def _get_dxx_studyed_member(stages_id, org_id, token=''):
    ...
    async with AsyncClient(headers={'token': token}, timeout=Timeout(10.0, connect=10.0)) as client:
        ...
        while True: #多次查询获取完整列表
            data = {"orgId": org_id, "stagesId": stages_id, "name": "", "tel": "", "pageNo": page_no, "pageSize": plugin_config.dxx_page_size}
...
async def get_dxx_studyed_member(username: str=plugin_config.dxx_username, password: str=plugin_config.dxx_password):
    ...
    db.set(const.dxx_studyed_list,json.dumps(member,ensure_ascii=False),ex=10*60)
    return member
...
async def read_members_info(path: str = path) -> dict:
    ...
    with open(path, encoding='utf-8') as f:
        ...
        db.set(const.member_list_filterd,json.dumps(members['filterd'],ensure_ascii=False),)
...
async def get_member_list():
    ...
    """
    global member_list
    if len(member_list['filterd'])>0 or len(member_list['other'])>0:
        return member_list
